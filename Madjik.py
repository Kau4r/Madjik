...
def init_db():
    ...
    # Patients table now includes medical_history
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS patients (
            ...
            medical_history TEXT  -- single editable field
        )
    ''')
    ...
    # ⚠️ Drop this since it is no longer needed
    # cursor.execute('''
...
def view_patient(patient_id):
    try:
        patient, records = get_patient_data(patient_id)
        if not patient:
